...
do_help() {
cat <<__EOT__
Usage: docker run [-v <option>] lfedge/eve [-f <fmt>] version|rootfs|live|verification_raw|verification_iso|verification_net

The artifact will be produced on stdout, so don't forget to redirect
it to a file or use the /out option below.

Example:
docker run --rm lfedge/eve verification_raw > verification.raw
...
Example:
docker run -v $HOME/eve-overrides:/in --rm lfedge/eve:latest verification_raw > verification.raw
...
Example:
docker run -v $HOME/eve-images:/out --rm lfedge/eve:latest verification_raw
...
Example:
docker run --rm lfedge/eve -f qcow2 verification_iso > eve-iso.img

The two raw formats "live" and "verification_raw" support an optional
